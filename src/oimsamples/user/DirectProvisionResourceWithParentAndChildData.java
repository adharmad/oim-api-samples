...
public class DirectProvisionResourceWithParentAndChildData {

	public static void main(String[] args) throws Exception {
		String userLogin = "johndoe2";
		String resourceName = "TestResource2";
		String childFormName = "UD_CBOEING";
		...
		long childFormDefKey = OIMUtils.getFormDefKey(fdIntf, childFormName);
		
		long oiuKey = usrIntf.provisionObject(usrKey, objKey);
		
		System.out.println("oiu key = " + oiuKey);

		long obiKey = OIMUtils.getObjectInstanceKey(usrIntf, usrKey, oiuKey);
		
		HashMap objData = new HashMap();
		objData.put("UD_OBOEING_PARFIELD1", "object parent field 1 data");
		objData.put("UD_OBOEING_PARFIELD2", "object parent field 2 data");
		
		fiIntf.setObjectFormData(obiKey, objData);
		
		long orcKey = OIMUtils.getProcessInstanceKey(usrIntf, usrKey, oiuKey);
		
		HashMap procData = new HashMap();
		procData.put("UD_BOEING_PARFIELD1", "process parent field 1 data");
		procData.put("UD_BOEING_PARFIELD2", "process parent field 2 data");
		
		fiIntf.setProcessFormData(orcKey, procData);
		
		for (int i=0 ; i<10 ; i++) {
			HashMap childData = new HashMap();
			childData.put("UD_CBOEING_CHILDFIELD1", "process childfield1 data " + i);
			childData.put("UD_CBOEING_CHILDFIELD2", "process childfield2 data " + i);
			
			fiIntf.addProcessFormChildData(childFormDefKey, orcKey, childData);
		}		
